...
describe "Staging schema from CQL" do
  dir = ENV['CQL_DIR'] || STG_CQL_DIR
  actual_dir = (ENV['CQL_DIR'] ? '' : STG_TEST_DIR+'/') + 'actual'
  expected_dir = (ENV['CQL_DIR'] ? '' : STG_TEST_DIR+'/') + 'expected'
  ...
  files.each do |cql_file|
    it "produces the expected Staging summary for #{cql_file}" do
      ...
      vocabulary.finalise
      compositor = ActiveFacts::Compositions::Staging.new(vocabulary.constellation, basename)
      compositor.generate
