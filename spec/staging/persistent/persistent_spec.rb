#
# Test the staging composition from CQL files by comparing generated Persistent Staging summary output
#
...
PERSTG_CQL_DIR = Pathname.new(__FILE__+'/../../../relational').relative_path_from(Pathname(Dir.pwd)).to_s
PERSTG_TEST_DIR = Pathname.new(__FILE__+'/..').relative_path_from(Pathname(Dir.pwd)).to_s
...
describe "Persistent Staging schema from CQL" do
  dir = ENV['CQL_DIR'] || PERSTG_CQL_DIR
  actual_dir = (ENV['CQL_DIR'] ? '' : PERSTG_TEST_DIR+'/') + 'actual'
  expected_dir = (ENV['CQL_DIR'] ? '' : PERSTG_TEST_DIR+'/') + 'expected'
