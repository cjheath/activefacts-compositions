...
RSpec::Matchers.define :be_like_stripped do |expected|
  match do |actual|
    actual_stripped = actual.gsub(/^.*Generated by ActiveFacts.*$/, '')
    expected_stripped = expected.gsub(/^.*Generated by ActiveFacts.*$/, '')
...
describe "SQL schema from CQL" do
  ...
  files.each do |cql_file|
    it "produces the expected SQL for #{cql_file}" do
      ...
      mode = {}
      compositor_klass_names = ['Relational']

      output = ActiveFacts::Generators::SQL.new([compositor.composition], mode, compositor_klass_names).generate
      ...
      if expected_text
        expect(output).to be_like_stripped(expected_text), "Output #{actual} doesn't match expected #{expected}"
      else
